...
- **Phase 4 — Theming + Builder UI (done).** `src/theme/Theming.ts` (palette swap, greeting split, clock, 30 s interval + focus). `src/ui/format.ts` (fmtClock, fmtTotal, escapeHtml shared by Builder + Runner). `src/ui/Builder.ts` — full Builder class: session name, view/edit segment rendering, numField (blur-reformat-in-place A11Y-3, type=text+inputmode=numeric for sec A11Y-4), soundPicker chips (aria-pressed + engine.preview()), total display, per-segment duplicate (corner button, inserts a flat copy after the source; always enabled) + remove (gated on >1), bookmark upsert, two-tap clear (3.5 s auto-cancel, outside-click cancel A11Y-5), Begin disabled at 0, bookmarked list (load/delete), tab-walk A11Y-1, toast. `main.ts` wires engine + builder + theme; Begin callback stubbed for Phase 5 Runner.
